...
const highlights: Array<{ title: string; text: string; icon: LucideIcon; gradient: string }> = [
  ...
  {
    title: "Konsultasi Terintegrasi",
    text: "Hubungkan langsung riwayat tumbuh kembang anak ke petugas Posyandu atau Puskesmas setempat.",
    icon: Stethoscope,
...
function HeroIllustration() {
  return (
    <div className="relative mx-auto w-full max-w-md" aria-hidden="true">
      ...
      <svg viewBox="0 0 400 360" fill="none" xmlns="http://www.w3.org/2000/svg" className="relative z-10 w-full drop-shadow-2xl animate-float-slow">
        {/* Background circle and grids */}
        <circle cx="200" cy="180" r="150" fill="url(#heroGrad)" opacity="0.08" />
        <circle cx="200" cy="180" r="110" fill="url(#heroGrad)" opacity="0.06" />

        {/* Outer glowing border ring */}
        <circle cx="200" cy="180" r="150" stroke="url(#strokeGrad)" strokeWidth="1" strokeDasharray="6 6" opacity="0.4" />

        {/* Growth chart bars */}
        <rect x="90" y="230" width="24" height="60" rx="6" fill="#10b981" opacity="0.2" />
        <rect x="130" y="210" width="24" height="80" rx="6" fill="#10b981" opacity="0.4" />
        <rect x="170" y="185" width="24" height="105" rx="6" fill="#06b6d4" opacity="0.6" />
        <rect x="210" y="160" width="24" height="130" rx="6" fill="#06b6d4" opacity="0.8" />
        ...
        {[
          ...
          <g key={i}>
            <circle cx={cx} cy={cy} r="8" fill="#10b981" opacity="0.15" className="animate-ping" style={{ animationDelay: `${i * 0.4}s` }} />
            <circle cx={cx} cy={cy} r="4" fill="#10b981" stroke="#fff" strokeWidth="2.5" />
          </g>
        ))}
        ...
        <g transform="translate(300,60)" className="animate-float-slow">
          <rect x="-30" y="-30" width="60" height="60" rx="16" fill="white" filter="url(#dropShadow)" />
          <rect x="-30" y="-30" width="60" height="60" rx="16" fill="rgba(254, 243, 199, 0.5)" />
          <path
        ...
        <g transform="translate(80,90)" className="animate-float-medium">
          <rect x="-25" y="-25" width="50" height="50" rx="14" fill="white" filter="url(#dropShadow)" />
          <rect x="-25" y="-25" width="50" height="50" rx="14" fill="rgba(236, 253, 245, 0.6)" />
          <path
            d="M0-8L-6-5V0C-6 4 0 7 0 7S6 4 6 0V-5L0-8Z"
            fill="#10b981"
          />
        </g>
        ...
        <g transform="translate(200,285)">
          <rect x="-65" y="-18" width="130" height="36" rx="18" fill="white" filter="url(#dropShadow)" />
          <circle cx="-42" cy="0" r="10" fill="#ecfeff" />
          <path d="M-45-2L-43 0L-39-4" stroke="#06b6d4" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill="none" />
          <text x="-25" y="4" fill="#0f172a" fontFamily="Outfit" fontSize="10" fontWeight="bold">Tinggi Sesuai</text>
        </g>
        ...
        <defs>
          ...
          <filter id="dropShadow" x="-20%" y="-20%" width="140%" height="140%">
            <feDropShadow dx="0" dy="4" stdDeviation="6" floodColor="#0e7490" floodOpacity="0.1" />
          </filter>
        </defs>
      </svg>
    </div>
...
export default function LandingPage() {
  ...
  return (
    <main className="min-h-screen bg-canvas text-slate-900 bg-dot-pattern">
      ...
      <nav className={`sticky top-0 z-40 transition-all duration-350 ${
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <a href="#hero" className="flex items-center gap-3">
            ...
            <span>
              <span className="block font-heading text-lg font-bold text-slate-950">StuntGuard</span>
              <span className="hidden text-[9px] font-bold uppercase tracking-[0.14em] text-cyan-600 sm:block">Healthcare Platform</span>
            </span>
          </a>
          <div className="hidden items-center gap-8 text-sm font-semibold text-slate-600 md:flex">
            <a href="#cek" className="transition hover:text-cyan-600">Cek Stunting</a>
            <Link
              to="/chatbot"
              onClick={(e) => {
                e.preventDefault();
            ...
            <a href="#tentang" className="transition hover:text-cyan-600">Tentang</a>
            <Link to="/login" className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-5 py-2.5 text-white hover:bg-slate-800 shadow-sm transition">
              <LogIn className="h-4 w-4" />
              Akses Portal Dashboard
            </Link>
          </div>
          <Link to="/login" className="inline-flex min-h-10 items-center rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-bold text-slate-700 md:hidden">
            Portal
          </Link>
        </div>
      </nav>
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 px-4 sm:px-6 lg:grid-cols-2 lg:gap-16 lg:px-8 items-center">
          <div className="flex flex-col justify-center text-left">
            ...
            <p className="mt-5 max-w-xl text-base leading-7 text-slate-600 sm:text-lg">
              StuntGuard menyediakan platform skrining awal risiko stunting yang cepat, ramah pengguna, dan berbasis data medis resmi secara instan.
            </p>
            ...
            <div className="mt-12 pt-8 border-t border-slate-200/60">
              <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Dipercaya Oleh Pengguna</p>
              <div className="mt-4 flex flex-wrap gap-x-8 gap-y-4 text-sm font-bold text-slate-600">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-slate-500">Akurasi Skrining Gizi</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-emerald-600 text-lg">50k+</span>
                  <span className="text-xs font-medium text-slate-500">Balita Terpantau</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-teal-600 text-lg">Real-time</span>
                  <span className="text-xs font-medium text-slate-500">Rekomendasi AI Gizi</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <section id="cek" className="relative pb-20 bg-gradient-to-b from-transparent to-white/70">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <h2 className="mt-3 font-heading text-3xl font-extrabold text-slate-950 sm:text-4xl">Kalkulator Gizi & Stunting</h2>
            <p className="mx-auto mt-3 max-w-2xl text-sm leading-relaxed text-slate-500">
              Isi data pertumbuhan di bawah ini dengan akurat. Pengukuran disesuaikan dengan kurva pertumbuhan standar organisasi kesehatan dunia (WHO).
            </p>
          </div>
          ...
          <div className="mt-10 overflow-hidden rounded-3xl border border-slate-200/60 bg-white/80 shadow-2xl shadow-cyan-900/[0.03] backdrop-blur-lg">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/50 px-6 py-4 sm:px-8">
              {[
                ...
                <div key={s.number} className="flex items-center gap-2.5">
                  ...
                  {s.number < 3 ? <ChevronRight className="hidden h-4 w-4 text-slate-300 sm:block" /> : null}
                </div>
              ))}
            </div>
            ...
            <form onSubmit={submit} className="p-6 sm:p-8">
              ...
              {step === 1 && (
                <div className="space-y-6">
                  <div className="text-center sm:text-left">
                    <p className="text-sm font-bold text-slate-900">Pilih Jenis Kelamin Anak</p>
                    <p className="text-xs text-slate-400 mt-1">Standar tinggi & berat badan WHO dibedakan berdasarkan jenis kelamin.</p>
                  </div>
                  <div className="grid gap-4 sm:grid-cols-2">
                    {[
                      { value: "female" as Gender, label: "Perempuan", subtitle: "Kurva Pertumbuhan Perempuan WHO", icon: UserRound, desc: "Pilih untuk anak perempuan", colorClass: "border-pink-500 bg-pink-50/20 text-pink-700 shadow-pink-100" },
                      { value: "male" as Gender, label: "Laki-laki", subtitle: "Kurva Pertumbuhan Laki-laki WHO", icon: Users, desc: "Pilih untuk anak laki-laki", colorClass: "border-cyan-500 bg-cyan-50/20 text-cyan-700 shadow-cyan-100" },
                    ].map((item) => {
                      ...
                      return (
                        ...
                        >
                          <span className={`rounded-xl p-3.5 transition ${selected ? "bg-cyan-600 text-white shadow-sm" : "bg-slate-100 text-slate-500"}`}>
                            <Icon className="h-6 w-6" />
                          </span>
                          <div>
                            <span className="block text-base font-bold text-slate-950">{item.label}</span>
                            <span className="block text-xs text-slate-400 mt-0.5">{item.subtitle}</span>
                          </div>
                        </button>
                      );
                    })}
              ...
              {step === 2 && (
                <div className="space-y-6">
                  <div className="text-center sm:text-left">
                    <p className="text-sm font-bold text-slate-900">Tentukan Usia Anak</p>
                    <p className="text-xs text-slate-400 mt-1">Masukkan usia balita dalam hitungan bulan (0 - 60 bulan).</p>
                  </div>
                  ...
                  <div className="rounded-2xl border border-slate-200/60 bg-slate-50/50 p-6 flex flex-col items-center justify-center text-center">
                    ...
                    <div className="mt-4">
                      <span className="font-heading text-4xl font-extrabold text-cyan-700">{form.age_month}</span>
                      <span className="text-lg font-bold text-slate-400 ml-1.5">bulan</span>
                    </div>
                    <p className="text-xs text-slate-500 mt-1">Setara dengan {Math.floor(form.age_month / 12)} tahun {form.age_month % 12} bulan</p>
                  </div>
              ...
              {step === 3 && (
                <div className="space-y-6">
                  <div className="text-center sm:text-left">
                    <p className="text-sm font-bold text-slate-900">Masukkan Tinggi & Berat Badan</p>
                    <p className="text-xs text-slate-400 mt-1">Ukur dengan cermat. Nilai standar ideal WHO akan ditampilkan sebagai pembanding.</p>
                  </div>
                    ...
                    <div className="rounded-2xl border border-slate-200/60 bg-slate-50/30 p-5 space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-bold text-slate-700">Tinggi Badan</span>
                        </div>
                        <div>
                          <span className="font-heading text-2xl font-extrabold text-cyan-700">{form.height_cm.toFixed(1)}</span>
                    ...
                    <div className="rounded-2xl border border-slate-200/60 bg-slate-50/30 p-5 space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-bold text-slate-700">Berat Badan</span>
                        </div>
                        <div>
                          <span className="font-heading text-2xl font-extrabold text-emerald-700">{form.weight_kg.toFixed(1)}</span>
      ...
      {/* ── Hasil Skrining ── */}
      {result ? (
        <section id="hasil" className="mx-auto max-w-5xl scroll-mt-16 px-4 py-12 sm:px-6 lg:px-8 border-t border-slate-100">
          <div className="text-center md:text-left mb-8">
            ...
            <h2 className="mt-2 font-heading text-3xl font-extrabold text-slate-950">Dashboard Penilaian Gizi Anak</h2>
            <p className="mt-2 text-sm text-slate-500">Hasil diagnosis model AI pendeteksi dini risiko stunting dan gizi buruk.</p>
          </div>
      ...
      {/* ── Kenapa StuntGuard ── */}
      <section id="tentang" className="border-t border-slate-100 bg-gradient-to-b from-white to-slate-50/50 py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <span className="text-xs font-bold uppercase tracking-wider text-cyan-600">Keunggulan Platform</span>
            ...
            <p className="mt-3 text-sm text-slate-500">Pantau secara berkelanjutan demi menjamin kebaikan gizi masa depan si kecil.</p>
          </div>
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {highlights.map(({ title, text, icon: Icon, gradient }) => (
              ...
              >
                <span className={`inline-flex rounded-2xl bg-gradient-to-br ${gradient} p-4 transition-transform duration-300 group-hover:scale-105`}>
                  <Icon className="h-6 w-6 text-cyan-700" />
                </span>
                <h3 className="mt-5 font-heading text-base font-bold text-slate-950">{title}</h3>
                <p className="mt-2.5 text-xs leading-5 text-slate-500">{text}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="rounded-3xl border border-amber-500/10 bg-amber-500/[0.03] p-6 text-amber-900 shadow-sm backdrop-blur-sm">
          <div className="flex gap-4 items-start">
            ...
            <div>
              <h3 className="font-bold text-sm text-amber-950">Catatan Penting (Disclaimer)</h3>
              <p className="mt-2 text-xs leading-relaxed text-amber-800">{DISCLAIMER}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer className="border-t border-slate-200/50 bg-white py-8">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 text-xs text-slate-400 sm:flex-row sm:items-center sm:justify-between sm:px-6 lg:px-8">
          <div className="flex items-center gap-2">
            <img src="/logo.png" alt="StuntGuard" className="h-6 w-6 object-contain opacity-80" />
            <p className="font-heading font-bold text-slate-700">StuntGuard Platform</p>
          </div>
